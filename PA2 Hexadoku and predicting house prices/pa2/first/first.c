...
/*int rowCheck(char** matrix);
int colCheck(char** matrix);
int gridCheck(char** matrix);
int isValid(char** matrix);
...
int colCheck(char **matrix){
  ...
  for(int i = 0 ; i <16; i++){
    for (int j = 0; j <16; j++){
      for(int k = 0; k<16; k++){
        if(matrix[i][j] != '-' && matrix[i][j]==matrix[k][j]){
          count++;
        }
        if (count==2){
          return 0;
...
int gridCheck(char **matrix){
  ...
  for(int i = 0 ; i <16; i++){
    for (int j = 0; j <16; j++){
      ...
      for(int m = 0; m < 4; m++){
	for(int n = 0; n < 4; n++){
          if(matrix[i][j] != '-' && matrix[i][j]==matrix[m+x][y+n]){
            count++;
          }
          if (count==2){
            return 0;
...
int isValid(char **matrix){
  if(rowCheck(matrix) == 1 && colCheck(matrix)==1 &&  gridCheck(matrix)==1){
    return 1;
  }
  return 0;
  }*/
